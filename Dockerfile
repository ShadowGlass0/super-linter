...
RUN wget --tries=5 -q -O /etc/apk/keys/sgerrand.rsa.pub https://alpine-pkgs.sgerrand.com/sgerrand.rsa.pub \
    ...
    && for PHAR_KEY_ID in \
	 ...
	 ; do \
      ...
      gpg --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys "$PHAR_KEY_ID" || \
      gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys "$PHAR_KEY_ID" || \
      gpg --keyserver keyserver.pgp.com --recv-keys "$PHAR_KEY_ID" || \
      gpg --keyserver hkp://pgp.mit.edu:80 --recv-keys "$PHAR_KEY_ID" \
      gpg --keyserver hkp://ipv4.pool.sks-keyservers.net --recv-keys "$PHAR_KEY_ID" ; \
    done \
    ...
    --target /usr/bin phpstan@^0.12.64 psalm@^4.12.0 phpcs@^3.5.8
